...
func TestGetSecret(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		assert.NotEmpty(t, secret.EncodedSecret)
...
func TestDeleteSecret(t *testing.T) {
	...
	// Test for non-existing secret ID. must be not found error
	assert.NoError(t, dbAdapter.DeleteSecret(t.Context(), userID, "non-existing-secret-y"))

	// delete a secret with an empty user ID
	err = dbAdapter.DeleteSecret(t.Context(), "", secretID)
	assert.NoError(t, err, "Expected no error when deleting a secret with empty user ID")
}
...
func TestCreateSecret(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		if assert.NoError(t, getErr) {
			...
			assert.Equal(t, newSecret.Username, savedSecret.Username)
...
func TestUpdateSecret(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		assert.NotEmpty(t, secret.EncodedUsername, "Expected a valid encoded username")
	...
	if assert.NoError(t, updateErr) {
		...
		if assert.NoError(t, getErr) {
			...
			assert.Equal(t, secret.Username, updatedSecret.Username)
