// Package session provides a Gorilla Sessions backend for SQLite.
/*
...
func NewSqliteStore(
	endpoint string, tableName string, path string, maxAge int, keyPairs ...[]byte,
) (*SqliteStore, error) {
...
func NewSqliteStoreFromConnection(
	db DB, tableName string, path string, maxAge int, keyPairs ...[]byte,
) (*SqliteStore, error) {
...
func (m *SqliteStore) Save(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	var err error
	if session.ID == "" {
		if err = m.insert(r.Context(), session); err != nil {
			return err
		}
	} else if err = m.save(r.Context(), session); err != nil {
		return err
	}
...
func (m *SqliteStore) insert(ctx context.Context, session *sessions.Session) error {
	...
	res, insErr := m.stmtInsert.ExecContext(ctx, encoded, createdOn, modifiedOn, expiresOn)
	if insErr != nil {
		return insErr
	}
	...
	session.ID = strconv.FormatInt(lastInserted, 10)
...
func (m *SqliteStore) Delete(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	...
	_, delErr := m.stmtDelete.ExecContext(r.Context(), session.ID)
	if delErr != nil {
		return delErr
	}
...
func (m *SqliteStore) save(ctx context.Context, session *sessions.Session) error {
	if session.IsNew {
	...
	_, updErr := m.stmtUpdate.ExecContext(ctx, encoded, createdOn, expiresOn, session.ID)
	if updErr != nil {
		return updErr
	}
...
func (m *SqliteStore) load(ctx context.Context, session *sessions.Session) error {
